...
namespace XrayUI.Services
{
    ...
    {
        /// <param name="target">
        /// The version actually being offered. No matching entry returns no notes.
        /// </param>
        /// <param name="language">
            ...
            ChangelogFeed? feed, Version target, string? language)
        {
            ...
            foreach (var entry in feed.Versions)
            {
                ...
                var preferRu = language?.StartsWith("ru", StringComparison.OrdinalIgnoreCase) == true;
                var preferred = Clean(preferRu ? entry.Ru : preferZh ? entry.Zh : entry.En);
                return preferred.Count > 0
                    ? preferred
                    : Clean(preferZh || preferRu ? entry.En : entry.Zh);
            }
